...
public interface RaceResultsRepository
    ...
    @Query(value = "SELECT raceresults.racename, raceresults.riderposition, UPPER(raceresults.ridername), riders.strava AS riderstrava FROM raceresults JOIN riders ON UPPER(raceresults.ridername) = UPPER(riders.ridername)  WHERE unaccent(raceresults.racename) ILIKE unaccent(:raceName) ORDER BY CAST(raceresults.riderposition AS INTEGER) ASC", nativeQuery = true)
